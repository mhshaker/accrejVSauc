...
def eRF_run(x_train, x_test, y_train, y_test, pram, unc_method, seed, predict=True): # x_test is x_pool
    ...
    np.random.seed(seed)
    model = None
    model = ExtraTreesClassifier(n_estimators=pram["n_estimators"], criterion='gini', max_depth=None, min_samples_split=2, 
    ...
    elif "skent" == unc_method:
        porb_matrix = model.predict_proba(x_test)
        total_uncertainty, epistemic_uncertainty, aleatoric_uncertainty = unc.sk_ent(np.array(porb_matrix))
    ...
    # elif "rl.aprox" == unc_method:
    #     porb_matrix_train = get_prob_matrixMDP(model, x_train, pram["n_estimators"], pram["laplace_smoothing"])
    #     porb_matrix_pool  = get_prob_matrixMDP(model, x_test, pram["n_estimators"], pram["laplace_smoothing"])
    #     porb_matrix_train = np.array(porb_matrix_train)
    #     porb_matrix_pool = np.array(porb_matrix_pool)
    #     y_train_onehot = tf.keras.utils.to_categorical(y_train, 2) # hard code the number of classes replace with len(np.unique(y_train)) but is slower
    #     total_uncertainty, epistemic_uncertainty, aleatoric_uncertainty = unc.unc_rl_prob(porb_matrix_train, porb_matrix_pool, y_train_onehot)
    ...
    elif "rl.linh" == unc_method:
        x = unc.EpiAle_Averaged_Support_Uncertainty(model, x_train, y_train, x_test, unc_mode, pram["n_estimators"])
        x = np.array(x)
        total_uncertainty     = x
        ...
        epistemic_uncertainty = x 
        aleatoric_uncertainty = x
    elif "rl.prow" == unc_method:
        x = unc.EpiAle_Averaged_Uncertainty_Preferences_Weight(model, x_train, y_train, x_test, unc_mode, pram["n_estimators"])
        x = np.array(x)
        total_uncertainty     = x
        epistemic_uncertainty = x 
        aleatoric_uncertainty = x
        ...
        for credal_index in range(pram["credal_size"]):
            ...
            model = ExtraTreesClassifier(n_estimators=pram["n_estimators"], criterion='gini', max_depth=None, min_samples_split=2, 
                                min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features='auto', 
                                max_leaf_nodes=None, min_impurity_decrease=0.0, min_impurity_split=None, 
                                bootstrap=False, oob_score=False, n_jobs=None, random_state=(seed+100) * credal_index, verbose=0, 
                                warm_start=False, class_weight=None, ccp_alpha=0.0, max_samples=None)
            model.fit(x_train, y_train)
...
def get_likelyhood(model_ens, x_train, y_train, n_estimators, laplace_smoothing, log=False):
    likelyhoods  = []
    for etree in range(n_estimators):
        if laplace_smoothing == 0:
            tree_prob_train = model_ens.estimators_[etree].predict_proba(x_train) 
        else:
            tree_prob_train = tree_laplace_corr(model_ens.estimators_[etree],x_train, laplace_smoothing)
...
def get_prob(model_ens, x_data, n_estimators, laplace_smoothing, log=False):
    prob_matrix  = []
    for etree in range(n_estimators):
        if laplace_smoothing == 0:
            tree_prob = model_ens.estimators_[etree].predict_proba(x_data) 
        else:
            tree_prob = tree_laplace_corr(model_ens.estimators_[etree],x_data, laplace_smoothing)
...
def tree_laplace_corr(tree, x_data, laplace_smoothing):
...
def get_prob_matrixMDP(model, x_test, n_estimators, laplace_smoothing, log=False): # model first data second prediction third
    porb_matrix = []
    for etree in range(n_estimators):
